		     ...
		     bool do_start = false)
{
  ...
  if (do_event) {
    if (sequence.size() + 1 != event_buffer.size()) {
      ERROR_MSG("reverse_cumsums: event_buffer size must be one longer than input's.");
    }
    event_buffer(n) = sum;
    for (int i = n - 1; i >= 0;  --i) {
  ...
  if (do_start) {
    if (sequence.size() + 1 != start_buffer.size()) {
      ERROR_MSG("reverse_cumsums: start_buffer size must be one longer than input's.");      
    }
    sum = 0.0;
    start_buffer(n) = sum;
    for (int i = n - 1; i >= 0;  --i) {
...
#endif
void forward_prework(const EIGEN_REF<Eigen::VectorXd> status,
...
double compute_sat_loglik(const EIGEN_REF<Eigen::VectorXi> first,
			  ...
			  const EIGEN_REF<Eigen::VectorXi> event_order,
			  const EIGEN_REF<Eigen::VectorXd> status,
...
void sum_over_events(const EIGEN_REF<Eigen::VectorXi> event_order,
                     ...
                     const EIGEN_REF<Eigen::VectorXd> status,
                     bool efron,
		     BUFFER_LIST(forward_cumsum_buffers), // List of vectors for scratch space.
...
double cox_dev(const EIGEN_REF<Eigen::VectorXd> eta, //eta is in native order  -- assumes centered (or otherwise normalized for numeric stability)
	       ...
	       const EIGEN_REF<Eigen::VectorXi> event_order,   
	       const EIGEN_REF<Eigen::VectorXi> start_order,
	       const EIGEN_REF<Eigen::VectorXd> status,        //everything below in event order
	       const EIGEN_REF<Eigen::VectorXi> first,
	       const EIGEN_REF<Eigen::VectorXi> last,
	       const EIGEN_REF<Eigen::VectorXd> scaling,
	       const EIGEN_REF<Eigen::VectorXi> event_map,
	       ...
	       bool efron = false)
{
  ...
  EIGEN_REF<Eigen::VectorXd> A_01 = forward_scratch_buffer; // Make a reference rather than a copy
  forward_cumsum(A_01, forward_cumsum_buffers0); // length=n+1 
  EIGEN_REF<Eigen::VectorXd> C_01 = forward_cumsum_buffers0; // Make a reference rather than a copy
  
  forward_prework(status, w_avg_buffer, scaling, risk_sums, 0, 2, forward_scratch_buffer, dummy, true);
  EIGEN_REF<Eigen::VectorXd> A_02 = forward_scratch_buffer; // Make a reference rather than a copy
  forward_cumsum(A_02, forward_cumsum_buffers1); // # length=n+1
  EIGEN_REF<Eigen::VectorXd> C_02 = forward_cumsum_buffers1; // Make a reference rather than a copy
  ...
  if (!efron) {
    ...
    EIGEN_REF<Eigen::VectorXd> A_11 = forward_scratch_buffer; // Make a reference rather than a copy
    forward_cumsum(A_11, forward_cumsum_buffers2); // # length=n+1
    EIGEN_REF<Eigen::VectorXd> C_11 = forward_cumsum_buffers2; // Make a reference rather than a copy

    forward_prework(status, w_avg_buffer, scaling, risk_sums, 2, 1, forward_scratch_buffer, dummy, true);
    EIGEN_REF<Eigen::VectorXd> A_21 = forward_scratch_buffer; // Make a reference rather than a copy
    forward_cumsum(A_21, forward_cumsum_buffers3); // # length=n+1
    EIGEN_REF<Eigen::VectorXd> C_21 = forward_cumsum_buffers3; // Make a reference rather than a copy

    forward_prework(status, w_avg_buffer, scaling, risk_sums, 2, 2, forward_scratch_buffer, dummy, true);
    EIGEN_REF<Eigen::VectorXd> A_22 = forward_scratch_buffer; // Make a reference rather than a copy
    forward_cumsum(A_22, forward_cumsum_buffers4); // # length=n+1
    EIGEN_REF<Eigen::VectorXd> C_22 = forward_cumsum_buffers4; // Make a reference rather than a copy
...
#endif
void hessian_matvec(const EIGEN_REF<Eigen::VectorXd> arg, // # arg is in native order
